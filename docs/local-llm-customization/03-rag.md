...
文書作成、コーディング、資料確認では、RAGが最も実用的なカスタマイズになることが多いです。手順書、仕様メモ、作業ログ、FAQなどをモデルに直接覚えさせるのではなく、必要なときに探して読ませます。

## RAGの基本構造

```mermaid
flowchart TD
  A["PDF・スライド・仕様書・作業メモ"] --> B["テキスト抽出"]
  B --> C["チャンク分割"]
  C --> D["埋め込みベクトル化"]
  D --> E["検索用DB"]
  F["質問"] --> G["関連チャンク検索"]
...
```text
この手順書の範囲だけを根拠に、確認問題を5問作ってください。
```

```text
この仕様メモ群から、未解決課題を3つに整理してください。
```

```text
作業メモを参照して、この設定を使い始めた理由を教えてください。
```

```text
この手順書と作業メモを参照して、利用者向けFAQを作ってください。
```
...
| メタデータ | 出典、日付、文書名など | 根拠提示とフィルタに重要 |
...
この指定は、文書やコードを扱う作業では特に重要です。手順や仕様について、根拠のない補完をされると困るからです。

## RAGとファインチューニングの違い

RAGは、知識を外部資料として参照する方法です。ファインチューニングは、モデルの振る舞いや出力傾向を変える方法です。

手順書や仕様書の中身を扱いたいなら、まずRAGです。
